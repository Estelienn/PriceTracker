document.addEventListener('DOMContentLoaded', () => {
    ...
    headings.forEach((heading: HTMLHeadingElement) => { heading.style.color = '#2E8B57'; });

    const mainElement = document.querySelector('main');
    if (mainElement) {
        const infoMsg: HTMLParagraphElement = document.createElement('p');
    ...
    if (localStorage.getItem('site-theme') === 'dark') document.body.classList.add('dark-theme');
    ...
    if (footerObj) footerObj.innerHTML += ` | Сьогодні: ${new Date().toLocaleDateString('uk-UA')}`;
    ...
    forms.forEach((form: HTMLFormElement) => {
        if (form.classList.contains('main-search')) return;
        ...
        form.addEventListener('submit', (event: SubmitEvent) => {
            ...
            const showError = (input: HTMLElement | null, message: string) => {
                if (!input) return;
                const inputEl = input as HTMLInputElement;
                inputEl.style.setProperty('border', '2px solid #FF5F56', 'important');
                let errorSpan = inputEl.nextElementSibling as HTMLElement;
            ...
            const clearError = (input: HTMLElement | null) => {
                if (!input) return;
                const inputEl = input as HTMLInputElement;
            ...
            const msgInput = form.querySelector('textarea[name="message"]') as HTMLTextAreaElement;
